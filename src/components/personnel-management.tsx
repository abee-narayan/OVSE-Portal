...
export function PersonnelManagement() {
  ...
  const [changeStep, setChangeStep] = useState(1) // 1: Verify Current, 2: New Details, 3: Verify New, 4: Success
  const [personToChange, setPersonToChange] = useState("")
  // Step 1
  const [currentEmailOtp, setCurrentEmailOtp] = useState("")
  const [currentAadhaarOtp, setCurrentAadhaarOtp] = useState("")
  ...
  const [changeNewAadhaar, setChangeNewAadhaar] = useState("")
  const [changeNewEmailOtp, setChangeNewEmailOtp] = useState("")
  const [changeNewAadhaarOtp, setChangeNewAadhaarOtp] = useState("")
  ...
  const handleChangeVerifyCurrent = () => {
  ...
  const handleChangeVerifyNew = () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setChangeStep(1)
            setCurrentEmailOtp("")
            setCurrentAadhaarOtp("")
            setChangeNewName("")
            setChangeNewEmail("")
            setChangeNewAadhaar("")
            setChangeNewEmailOtp("")
            setChangeNewAadhaarOtp("")
            setPersonnel(prev => prev.map(p => {
                if (p.id === personToChange) {
                    return { ...p, name: changeNewName, email: changeNewEmail }
                }
                return p
            }))
        }, 2000)
                ...
                <div className="space-y-4">
                    {personnel.map(p => (
                        <div key={p.id} className="flex items-center justify-between p-4 border rounded-xl hover:bg-slate-50 transition-colors">
                            ...
                            {currentUserRole === "MPOC" && (
                                <Dialog open={isChangeOpen && personToChange === p.id} onOpenChange={(open) => {
                                    setIsChangeOpen(open);
                                    if (open) setPersonToChange(p.id);
                                }}>
                                    <DialogTrigger asChild>
                                        <Button 
                                            variant="ghost" 
                                    ...
                                    <DialogContent className="sm:max-w-[425px]">
                                        <DialogHeader>
                                            <DialogTitle>Change Personnel Details</DialogTitle>
                                            <DialogDescription>
                                                Replace {p.name} with a new individual. This requires multiple levels of verification.
                                            </DialogDescription>
                                        </DialogHeader>
                                            ...
                                            <div className="py-4 space-y-6">
                                                <div className="space-y-4">
                                                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex gap-2">
                                                        ...
                                                        <p className="text-xs text-blue-800">Step 1: Verify current personnel details. OTPs have been sent to their registered email and Aadhaar mobile.</p>
                                                    </div>
                                                    <div>
                                                        <Label className="flex items-center gap-2 mb-2"><Mail className="h-4 w-4" /> Current Email OTP</Label>
                                                        <Input 
                                                            placeholder="Enter 6-digit OTP" 
                                                            maxLength={6} 
                                                    ...
                                                    <div>
                                                        <Label className="flex items-center gap-2 mb-2"><Smartphone className="h-4 w-4" /> Current Aadhaar OTP</Label>
                                                        <Input 
                                                            placeholder="Enter 6-digit OTP" 
                                                            maxLength={6} 
                                                            value={currentAadhaarOtp}
                                                            onChange={(e) => setCurrentAadhaarOtp(e.target.value)}
                                                        />
                                                    </div>
                                                </div>
                                                <DialogFooter>
                                                    <Button 
                                                        onClick={handleChangeVerifyCurrent}
                                                        disabled={currentEmailOtp.length !== 6 || currentAadhaarOtp.length !== 6 || isVerifyingCurrent}
                                        ...
                                        {changeStep === 2 && (
                                            <div className="py-4 space-y-4">
                                                <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex gap-2 mb-4">
                                                    <Users className="h-5 w-5 text-blue-600 shrink-0" />
                                                    <p className="text-xs text-blue-800">Step 2: Enter the details of the new personnel.</p>
                                                </div>
                                                <div className="space-y-4">
                                                    <div>
                                                        <Label className="mb-2">New Full Name</Label>
                                                        <Input value={changeNewName} onChange={e => setChangeNewName(e.target.value)} placeholder="Aadhaar Matched Name" />
                                                    </div>
                                                    <div>
                                                        <Label className="mb-2">New Official Email</Label>
                                                        <Input value={changeNewEmail} onChange={e => setChangeNewEmail(e.target.value)} placeholder="new.contact@organization.gov.in" />
                                                    </div>
                                                    <div>
                                                        <Label className="mb-2">New Aadhaar Number</Label>
                                                        <Input value={changeNewAadhaar} onChange={e => setChangeNewAadhaar(e.target.value)} placeholder="1234 5678 9012" maxLength={14} />
                                                    </div>
                                                </div>
                                                <DialogFooter>
                                                    <Button 
                                                        onClick={() => setChangeStep(3)}
                                                        disabled={!changeNewName || !changeNewEmail || changeNewAadhaar.length < 12}
                                        ...
                                        {changeStep === 3 && (
                                            <div className="py-4 space-y-6">
                                                <div className="space-y-4">
                                                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex gap-2">
                                                        <ShieldAlert className="h-5 w-5 text-blue-600 shrink-0" />
                                                        <p className="text-xs text-blue-800">Step 3: Verify new personnel details. OTPs have been sent to their email and Aadhaar mobile.</p>
                                                    </div>
                                                    <div>
                                                        <Label className="flex items-center gap-2 mb-2"><Mail className="h-4 w-4" /> New Email OTP</Label>
                                                        <Input 
                                                            placeholder="Enter 6-digit OTP" 
                                                            maxLength={6} 
                                                            value={changeNewEmailOtp}
                                                            onChange={(e) => setChangeNewEmailOtp(e.target.value)}
                                                        />
                                                    </div>
                                                    <div>
                                                        <Label className="flex items-center gap-2 mb-2"><Smartphone className="h-4 w-4" /> New Aadhaar OTP</Label>
                                                        <Input 
                                                            placeholder="Enter 6-digit OTP" 
                                                            maxLength={6} 
                                                            value={changeNewAadhaarOtp}
                                                            onChange={(e) => setChangeNewAadhaarOtp(e.target.value)}
                                                        />
                                                    </div>
                                                </div>
                                                <DialogFooter>
                                                    <Button 
                                                        onClick={handleChangeVerifyNew}
                                                        disabled={changeNewEmailOtp.length !== 6 || changeNewAadhaarOtp.length !== 6 || isVerifyingNew}
                                                        className="w-full bg-blue-600 hover:bg-blue-700"
                                                    >
                                                        {isVerifyingNew ? "Verifying..." : "Verify & Complete Change"}
                                                    </Button>
                                                </DialogFooter>
                                            </div>
                                        )}
                                        ...
                                        {changeStep === 4 && (
                                            <div className="py-8 flex flex-col items-center justify-center space-y-4">
                                                ...
                                                <h3 className="font-bold text-lg">Personnel Changed</h3>
                                                <p className="text-sm text-center text-slate-500">The details have been successfully updated.</p>
                                            </div>
                                        )}
                                    </DialogContent>
                                </Dialog>
